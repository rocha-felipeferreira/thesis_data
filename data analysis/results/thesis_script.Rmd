...
## Parei aqui
mapas_antes_gf <- map_data(map = "world")
mapas_antes_gf <- filter(mapas_antes_gf, region != "Antarctica")

setdiff(mencoes_paises_antes_gf$full_name, mapas_antes_gf$region)

mencoes_paises_antes_gf$full_name <- str_replace_all(mencoes_paises_antes_gf$full_name, pattern = "United States", "USA")

mencoes_paises_antes_gf$full_name <- str_replace_all(mencoes_paises_antes_gf$full_name, pattern = "United Kingdom", "UK")

mencoes_paises_antes_gf$full_name <- str_replace_all(mencoes_paises_antes_gf$full_name, pattern = "Vatican City", "Vatican")

mencoes_paises_antes_gf$full_name <- str_replace_all(mencoes_paises_antes_gf$full_name, pattern = "São Tomé & Príncipe", "Sao Tome and Principe")

mencoes_paises_antes_gf$full_name <- str_replace_all(mencoes_paises_antes_gf$full_name, pattern = "Palestinian Territories", "Palestine")

mencoes_paises_antes_gf$full_name <- str_replace_all(mencoes_paises_antes_gf$full_name, pattern = "Antigua & Barbuda", "Antigua")

mencoes_paises_antes_gf$full_name <- str_replace_all(mencoes_paises_antes_gf$full_name, pattern = "Bosnia & Herzegovina", "Bosnia and Herzegovina")

setdiff(mencoes_paises_antes_gf$full_name, mapas_antes_gf$region)

mapas_antes_gf <- left_join(mapas_antes_gf, mencoes_paises_antes_gf, by = c("region" = "full_name"))

mapas_antes_gf[is.na(mapas_antes_gf)] <- 0

mapas_antes_gf$valor_ordenado <- cut(mapas_antes_gf$value, breaks = c(0, 1, 50, 100, 225, 589), labels = c("0 menções","1-50 menções", "50 a 100 menções", "100 a 225 menções", "225 a 589"), include.lowest = TRUE)

ggplot(data = mapas_antes_gf, aes(x = long, y = lat, group = group, fill = valor_ordenado)) +
  geom_polygon(colour = "black") + 
  scale_fill_manual(values = brewer.pal(5, "BuPu")[1:5]) +
