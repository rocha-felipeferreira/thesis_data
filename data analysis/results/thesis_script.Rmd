...
main_corpus <- corpus_data_source %>% drop_na() %>% corpus() #ver isso aqui!
ndoc(main_corpus)
...
set.seed(202438)
seta <- grid::arrow(type = "closed", length = unit(.08, "inches"))

ggraph(proper_nouns_ngram_plot, layout = "fr") + geom_edge_link(aes(edge_alpha = count), show.legend = FALSE, arrow = seta, end_cap = circle(.03, "inches")) +
  geom_node_point(size = 3, colour = "lightblue") + geom_node_text(aes(label = name), vjust = 1, hjust = 1) + theme_void()
...
realm_keyness_exec_plot <- realm_keyness_exec + thesis_theme() + 
  ...
  scale_color_manual(labels = c("Internacional", "Doméstico"), values = c("#3e424a", "gray")) 
...
ggplot(feat_position, aes(x = PRES, y = MRE)) + 
...
speakers_keyness_plot <- speakers_keyness_plot + thesis_theme() + 
  scale_x_continuous(expand = c(0,0), limits = c(-200, 300)) +
  scale_color_manual(labels = c("PRES", "MRE"), values = c("#3e424a", "gray"))
...
brazil_defends_mre_kwic$id <- "MRE"
...
brazil_defends_pres_kwic$id <- "PRES"

# Merged

brazil_defends_kwic <- rbind(brazil_defends_mre_kwic, brazil_defends_pres_kwic)
...
brazil_mustshould_mre_kwic$id <- "MRE"
...
brazil_mustshould_pres_kwic$id <- "PRES"

# Merged

brazil_mustshould_kwic <- rbind(brazil_mustshould_mre_kwic, brazil_mustshould_pres_kwic)
...
brazil_needs_pres_kwic$id <- "PRES"

# Merged

brazil_needs_kwic <- rbind(brazil_needs_mre_kwic, brazil_needs_pres_kwic)
...
brazil_can_pres_kwic$id <- "PRES"

# Merged

brazil_can_kwic <- rbind(brazil_can_mre_kwic, brazil_can_pres_kwic)
...
kwic_plot <- ggplot(kwic_graph_df, aes(x = sentence, y = valores)) +
...
hist(mapa_pres$value)
quantile(mapa_pres$value)
summary(mapa_pres$value)

mapa_pres$valor_cat <- cut(mapa_pres$value, breaks = c(0, 1, 50, 250, 500, 10614), labels = c("0", "1-50", "50-250", "250-500", "1000 ou mais"), include.lowest = TRUE)

ggplot(data = mapa_pres, aes(x = long, y = lat, group = group, fill = valor_cat)) +
  geom_polygon(colour = "black") + 
  coord_cartesian() + theme_void() +
  labs(fill = "Número de Citações") + 
  scale_fill_viridis_d(option = "D", direction = 1, alpha = .8)
...
hist(mapa_mre$value)
quantile(mapa_mre$value)
summary(mapa_mre$value)

mapa_mre$valor_cat <- cut(mapa_mre$value, breaks = c(0, 1, 50, 250, 500, 8946), labels = c("0", "1-50", "50-250", "250-500", "1000 ou mais"), include.lowest = TRUE)

ggplot(data = mapa_mre, aes(x = long, y = lat, group = group, fill = valor_cat)) +
  geom_polygon(colour = "black") + 
  coord_cartesian() + theme_void() +
  labs(fill = "Número de Citações") + 
  scale_fill_viridis_d(option = "D", direction = 1, alpha = .8)
...
agents_similarity <- dfm(agents_similarity_tokens, tolower = TRUE, 
                          ...
                          remove_symbols = TRUE, remove_separators = TRUE,
                          remove = c(stopwords(language = "portuguese"), 
                                     custom_stopwords)) %>% dfm_wordstem(language = "portuguese")
...
agents_similarity_dend <- agents_similarity %>% as.dist() %>% hclust(method = "average") 

plot(agents_similarity_dend, main = "Distância Euclidiana em Documentos Normalizados", ylab = "", xlab = "", axes = FALSE, sub = "")

dotchart(as.list(agents_similarity)$"Lafer", xlab = "Distância Euclidiana", pch = 19)
