...
proper_nouns_plot <- ggraph(proper_nouns_ngram_plot, layout = "fr") + geom_edge_link(aes(edge_alpha = count), show.legend = FALSE, arrow = seta, end_cap = circle(.05, "inches")) +
  geom_node_point(size = 3, colour = "lightblue") + geom_node_text(aes(label = name), vjust = 1, hjust = 1, size = 3.5, nudge_x = .2, nudge_y = -.2) + theme_void()

# ggsave("collocation_exec.png", proper_nouns_plot, height = 6.2, width = 10.2, dpi = 600)
...
mapa_pres$valor_cat <- cut(mapa_pres$value, breaks = c(0, 1, 50, 250, 500, 10653), labels = c("0", "1-50", "50-250", "250-500", "Mais de 500"), include.lowest = TRUE)
...
ggplot(data = mapa_pres, aes(x = long, y = lat, group = group, fill = valor_cat)) +
  ...
  scale_fill_manual(values = carto.pal(pal1 = "sand.pal", n1 = 5, middle = TRUE, transparency = TRUE))
...
mapa_mre$valor_cat <- cut(mapa_mre$value, breaks = c(0, 1, 50, 250, 500, 9046
), labels = c("0", "1-50", "50-250", "250-500", "Mais de 500"), include.lowest = TRUE)
...
dfm_specific_terms <- dfm_group(dfm_specific_terms, groups = "speaker_reshaped") %>% dfm_tfidf()
